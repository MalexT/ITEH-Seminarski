...
if($id == "0"){
  ...
  $q = "SELECT * FROM prijava p join predmet pre on p.predmetID = pre.predmetID join student s on p.brojIndeksa=s.brojIndeksa join rok r on p.rokID=r.rokID join sluzbenik sl on p.sluzbenikID = sl.sluzbenikID where r.rokID=$id";
}
